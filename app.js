...
function App() {
    ...
    const [city,setcity] = useState("Enter The Location")
    const [desc,setdesc] = useState("Loading...")
    const [enteredvalue,setenteredvalue] = useState("Enter the value")
    const [feelslike,setfell]= useState("0")
    ...
    function getData()
    {
        ...
        weather.then(function(dalta){
            const roundedTemp = Math.round(dalta.data.main.temp - 273);
            const roundfellslike =Math.round(dalta.data.main.feels_like -273)
            setdeg(roundedTemp)
            setdesc(dalta.data.weather[0].main)
            setcity(dalta.data.name)
            setfell(roundfellslike)
        })
    }
    return (
        <div className="flex flex-row justify-center h-[100vh] items-center " >
            ...
            <div style={{backgroundImage: "linear-gradient(120deg, #a6c0fe 0%, #f68084 100%) "}} className="p-2 rounded-md shadow ">
                ...
                <p className="text-xs mt-2">Degree: {deg}°C | City or State: {city} | Weather: {desc} | Fells Like: {feelslike}°C</p>
            </div>
